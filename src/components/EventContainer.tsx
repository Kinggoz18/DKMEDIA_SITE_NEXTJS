...
export default function EventContainer(props: EventContainerProps) {
  ...
  return (
    <div className='relative flex flex-col w-[82%] h-fit min-w-[200px] min-h-[427px] items-start gap-y-[15px] duration-300 ease-in-out pb-[20px]' ref={ref}>
      <div className='relative w-[90%]'>
        <h3 className='h3-small w-full text-wrap text-left'>{title}</h3>
        <h4 className='h4-small'>{formatedDate}</h4>
      </div>

      <img src={image} alt={`Event ${title} image`} className='relative w-full h-[260px]' />
